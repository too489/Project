...
using namespace std;
static int TotalSize = 0,printcount=0;
...
void treePrint(node* nd[], int n)
{	   /*프린터를 정방향으로 하기위해서 마지막줄의 최대 갯수인 n을 받아왔습니다
	   그러고 그 n을 현재 줄에 출력되는 값에 따라 나눠 그 값을 간격으로 주었고 그것을
	   출력하면서 각줄의 노드가 전부 출력되면 다음 줄을 출력하게 만들었습니다*/
		if (nd != NULL) {
			int i=0,l,div=1;
			while(true) {
				for (l = 0; l < div; l++) {
					cout << setw(n/(div+1)) << "" << nd[i]->data() ;
					i++;
					if (i > printcount)
						break;
				} 
...
int main() {
	...
	/*마지막줄의 길이를 알아보고 그길이를 print함수에 줬습니다*/
	treePrint(nd, n+1);
	cout << endl;
	system("pause");
	return 0;
